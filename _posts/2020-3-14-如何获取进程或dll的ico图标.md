...
其实`ico`格式文件，是一组图片集合，也就是对应系统api中的：`ImageList`。我们可以任意找一个ico图标，我们会发现ico图标的大小会比通常的`jpg`,`png`等格式的图片更大。我们用`visual studio`打开查看，我们会发现，其实`ico`图标里面是有一组图标，可以是`png`、`jpg`、`bmp`等格式。分辨率包含有：`16 * 16`、`24 * 24`、`32 * 32`、`48 * 48`、`64 * 64`、` 128 * 128`,甚至是`256 * 256`！**难怪可以这么清晰！** 
